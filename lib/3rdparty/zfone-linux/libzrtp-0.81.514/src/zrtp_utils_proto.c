		...
		if (ZRTP_PKTYPE_PRESH == stream->pubkeyscheme->base.id)
		{
			if (_is_presh_in_hello(&stream->messages.peer_hello) >= 0)
			{
				...
				if (session->secrets.rs1->_cachedflag && verifiedflag) {
					ZRTP_LOG(3,(_ZTU_,"\tDefine stream mode: user wants PRESHARED and we"
								" have RS1, so use it. ID=%u\n", stream->id));
					return ZRTP_STREAM_MODE_PRESHARED;
				} else {
					ZRTP_LOG(3,(_ZTU_,"\tDefine stream mode: user wants PRESHARED but we"
								"HAVE RS1=0 or V=0. ID=%u\n", stream->id));
					ZRTP_LOG(3,(_ZTU_,"\tDefine stream mode: set pkt=DH3K and use DH mode.\n"));
				}
			}
		}
