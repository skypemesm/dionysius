...
/*----------------------------------------------------------------------------*/
extern void zrtp_init_rng(zrtp_global_t* zrtp);

extern zrtp_status_t zrtp_defaults_sas(zrtp_global_t* global_ctx);
// Digital Signatures not yet available
//extern zrtp_status_t zrtp_defaults_sig(zrtp_global_t* global_ctx);
...
zrtp_status_t zrtp_init(zrtp_config_t* config, zrtp_global_t** zrtp)
{
	...
	ZRTP_LOG(3, (_ZTU_,"INITIALIZING LIBZRTP: clientif=%.16s P=%d, M=%d..\n",
				 new_zrtp->client_id.buffer, config->lic_mode, config->is_mitm));
	...
	// Digital Signatures not yet available
	//init_mlist(&new_zrtp->_sigtype_head);

    /* Init RNG context */
    zrtp_init_rng(new_zrtp);
   	
	/* Initialize SRTP engine */
	s =  zrtp_srtp_init(new_zrtp);
	if (zrtp_status_ok != s) {		
		ZRTP_LOG(1, (_ZTU_,"ERROR! SRTP initialization failed with error code <%s>\n", zrtp_log_status2str(s)));
		return zrtp_status_fail;
    }    

	if (new_zrtp->cb.cache_cb.on_init) 
	{
		ZRTP_LOG(1, (_ZTU_,"Default cache name:%s:%d.\n", config->def_cache_path.buffer, config->def_cache_path.length));
		s = new_zrtp->cb.cache_cb.on_init(new_zrtp);
		if (zrtp_status_ok != s) {
			ZRTP_LOG(1, (_ZTU_,"ERROR! cache on_init() callback failed <%s>\n", zrtp_log_status2str(s)));
			if (new_zrtp->cb.cache_cb.on_down) {
				new_zrtp->cb.cache_cb.on_down(new_zrtp);
			}
	...
	if (new_zrtp->cb.sched_cb.on_init) 
	{
		...
		if (zrtp_status_ok != s) {
			...
			if (new_zrtp->cb.sched_cb.on_down) {
				new_zrtp->cb.sched_cb.on_down();
			}
			zrtp_srtp_down(new_zrtp);
			return zrtp_status_fail;
		}
...
zrtp_status_t zrtp_down(zrtp_global_t* zrtp)
{
	...
	// Digital Signatures not yet available
	// zrtp_comp_done(ZRTP_CC_SIG, zrtp);
...
zrtp_status_t zrtp_comp_done(zrtp_crypto_comp_t type, zrtp_global_t* zrtp)
{
	...
	// Digital Signatures not yet available
	//case ZRTP_CC_SIG:
	//    DESTROY_COMP(node, tmp, zrtp_sig_scheme_t, &zrtp->_sigtype_head, comp);
	case ZRTP_CC_CIPHER:
								  ...
								  zrtp_global_t* zrtp )
{
	...
	// Digital Signatures not yet available
	//case ZRTP_CC_SIG:
	//	ZRTP_COMP_INIT(zrtp_sig_scheme_t, &zrtp->_sigtype_head, comp);
	case ZRTP_CC_CIPHER:
...
void* zrtp_comp_find(zrtp_crypto_comp_t type, uint8_t id, zrtp_global_t* zrtp)
{
	...
	// Digital Signatures not yet available
	//case ZRTP_CC_SIG:
  	//	ZRTP_COMP_FIND(&zrtp->_sigtype_head, id, zrtp_sig_scheme_t, res);
	case ZRTP_CC_CIPHER:
...
{    
	...
	//	switch (id)
	//	{
	//	case ZRTP_SIGTYPE_EC256P: return ZRTP_EC256P;
	//	case ZRTP_SIGTYPE_EC384P: return ZRTP_EC384P;
	//	case ZRTP_SIGTYPE_EC521P: return ZRTP_EC521P;
	//	default: return "Unkn";
	//	}
...
#if (defined(ZRTP_USE_ENTERPRISE) && (ZRTP_USE_ENTERPRISE == 1))
		case ZRTP_PKTYPE_DH4096: return ZRTP_DH4K;
...
#if (defined(ZRTP_USE_ENTERPRISE) && (ZRTP_USE_ENTERPRISE == 1))
		if (!zrtp_memcmp(ZRTP_DH4K, name, ZRTP_COMP_TYPE_SIZE)) {
			return ZRTP_PKTYPE_DH4096;
		}
