...
{
	strcpy(path, "./zrtp_test_cache.dat");
}
	...
	{		
		...
		for (i=0; i<session->streams_count; i++)
		{
			...
			if (packet->is_rtp)
			{
				...
				rtp_hdr->ts = zrtp_hton32(zrtp_time_now()/1000);
				...
				zrtp_memcpy(packet->body + sizeof(zrtp_rtp_hdr_t), word, (uint32_t)strlen(word));
	  			packet->length = sizeof(zrtp_rtp_hdr_t) + strlen(word);
			...
			else
			{
				...
				zrtp_memcpy(packet->body + sizeof(zrtp_rtcp_hdr_t) + 4, word, strlen(word));
				packet->length = sizeof(zrtp_rtcp_hdr_t) + strlen(word) + 4;
							  ...
							  zrtp_test_channel_id_t* chan_id)
{
	...
	do {
		...
		zrtp_profile.pk_schemes[0] = ZRTP_PKTYPE_EC256P;
		zrtp_profile.pk_schemes[1] = ZRTP_PKTYPE_DH3072;
		zrtp_profile.pk_schemes[2] = ZRTP_PKTYPE_DH2048;
...
#if (ZRTP_ENABLE_TEST == 1)	
		zrtp_profile.autosecure = 1;
		zrtp_profile.pk_schemes[0] = ZRTP_PKTYPE_DH2048;
		zrtp_profile.pk_schemes[1] = ZRTP_PKTYPE_DH3072;
