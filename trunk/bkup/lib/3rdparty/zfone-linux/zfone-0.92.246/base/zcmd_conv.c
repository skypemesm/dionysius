...
#define CONVINT(val, is_htonl) is_htonl ? htonl(val) : ntohl(val)
#define CONVINT_(val, is_htonl) val = is_htonl ? htonl(val) : ntohl(val)
#define CONVSHORT(val, is_htonl) is_htonl ? htons(val) : ntohs(val)
#define CONVSHORT_(val, is_htonl) val = CONVSHORT(val, is_htonl)

//------------------------------------------------------------------------------
static void convert(zrtp_cmd_t* cmd, int is_htonl)
{
    unsigned int  code = is_htonl ? cmd->code : ntohl(cmd->code);

    // abstract command part convertion
    cmd->stream_id	= CONVSHORT(cmd->stream_id, is_htonl);
	...
	case UPDATE_STREAMS:
	{
        ...
        case SET_VERIFIED:
        {
	    zrtp_cmd_set_verified_t* verefied_cmd = (zrtp_cmd_set_verified_t*) cmd->data;
	    verefied_cmd->is_verified	= CONVINT(verefied_cmd->is_verified, is_htonl);
        }break;
        case SET_NAME:
        {
	    zrtp_cmd_set_name_t* name_cmd = (zrtp_cmd_set_name_t*) cmd->data;
	    name_cmd->name_length	= CONVINT(name_cmd->name_length, is_htonl);
        }break;
        case SEND_VERSION:
        {
	    zrtp_cmd_send_version_t* version_cmd = (zrtp_cmd_send_version_t*) cmd->data;
        ...
        case ZRTP_PACKET:
        {
	    zrtp_cmd_zrtp_packet_t* packet_cmd = (zrtp_cmd_zrtp_packet_t*) cmd->data;
	...
	case PLAY_ALERT:
	{
	    zrtp_cmd_alert_t* alert_cmd = (zrtp_cmd_alert_t*) cmd->data;
	...
	{
		int i;
		zfone_params_t* c = (zfone_params_t*)(&cmd->data);
	...
	case SET_CACHE_INFO:
	{
		zrtp_cache_info_t* info = (zrtp_cache_info_t*)(&cmd->data);
