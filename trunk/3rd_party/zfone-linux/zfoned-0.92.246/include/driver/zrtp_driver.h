 ...
 * 
 * Andrew Rozinko, mailto:a.rozinko@soft-industry.com
 */

#ifndef __zrtp_driver_h__
#define	__zrtp_driver_h__

#ifdef NDIS51_MINIPORT
#define ZRTP_MAJOR_NDIS_VERSION            5
#define ZRTP_MINOR_NDIS_VERSION            1
#else
#define ZRTP_MAJOR_NDIS_VERSION            4
#define ZRTP_MINOR_NDIS_VERSION            0
#endif

#ifdef NDIS51
#define ZRTP_PROT_MAJOR_NDIS_VERSION    5
#define ZRTP_PROT_MINOR_NDIS_VERSION    0
#else
#define ZRTP_PROT_MAJOR_NDIS_VERSION    4
#define ZRTP_PROT_MINOR_NDIS_VERSION    0
#endif

#define ZRTP_DRIVER_VERSION		0x0080
...
#define ETHERNET_HEADER_SIZE     (sizeof(struct eth_hdr))
#define ETHERNET_PACKET_SIZE     3072

//advance declaration
typedef struct _ADAPT ADAPT, *PADAPT;

extern KEVENT g_eventStop;
extern zrtp_zid_t			zid;
extern zrtp_global_t		*zrtp_global;



typedef enum zfone_process_packet_status
{	
	ZFONE_PROCESS_PACKET_RES_DROP		= 0,
	ZFONE_PROCESS_PACKET_RES_FORWARD	= 1,
	ZFONE_PROCESS_PACKET_RES_SEND		= 2
} zfone_process_packet_status_t;

/*!
 * \brief Process IP packet with ZRTP kernel
 * Zfone kernel copys body of PPacket to the buffer and then try to process this packet.
 * param pPacket - source packet for processing
 * param buffer  - buffer for result, processed packet storing
 * param buffer_length - packet buffer length
 * param changed - packet modification flag. if after function call value of this flag is
 *                 equal to 1 - packet have been changed. You need to reconstruct packet
 *				   and send it back to the network.
 * param zrtp_status - libzrtp stores result of zrtp_process_Xxx() functions to this variable
 */
NDIS_STATUS ProcessPacket( PADAPT pAdapt,
						   PNDIS_PACKET pPacket,
						   char *buffer,
						   uint32_t *buffer_length,
						   zfone_process_packet_status_t* zstatus,
						   int needLogs );

int init_pipeline(LONG limit);
void done_pipeline();
void add_packet( zfone_packet_t* packet, zrtp_voip_proto_t proto);

void init_notify();
void deinit_notify();

int is_zrtp_ok();
void set_zrtp_ok(int value);
int get_zrtp_status();

int get_zid(const char *path, zrtp_zid_t zid, int force);

int zfone_get_config_state();
void zfone_set_config_state(int value);
