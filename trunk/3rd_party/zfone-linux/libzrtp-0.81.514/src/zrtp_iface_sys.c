...
zrtp_status_t zrtp_sem_init(zrtp_sem_t** sem, uint32_t value, uint32_t limit)
{
    ...
    sprintf(name_buff, "/libzrtp.%lxZ%lx", now/1000, now);
    ...
    if ((sem_t *)SEM_FAILED == new_sem->sem) {
        if (errno == ENAMETOOLONG) {
            ...
            sprintf(name_buff, "/libMPC.%lxZ%lx", now, now/1000);
        } else {
			s = zrtp_status_fail;
        }
        new_sem->sem = sem_open(name_buff, O_CREAT | O_EXCL, 0644, value);
...
/*---------------------------------------------------------------------------*/
#elif (ZRTP_PLATFORM == ZP_LINUX) || (ZRTP_PLATFORM == ZP_DARWIN) || (ZRTP_PLATFORM == ZP_SYMBIAN)
