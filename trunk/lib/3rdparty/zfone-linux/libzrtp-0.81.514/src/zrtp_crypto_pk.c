...
static zrtp_status_t zrtp_dh_init(void *s)
{
	...
	switch (self->base.id) {
		...
		case ZRTP_PKTYPE_DH4096:
			p = &self->base.zrtp->P_4096;
			p_1 = &self->base.zrtp->P_4096_1;
			p_data = self->base.zrtp->P_4096_data;
			p_data_length = sizeof(self->base.zrtp->P_4096_data);
			break;
		default:
...
static zrtp_status_t zrtp_dh_free(void *s)
{
	...
	switch (self->base.id) {
		...
		case ZRTP_PKTYPE_DH3072:
			...
			break;
		case ZRTP_PKTYPE_DH4096:
			bnEnd(&self->base.zrtp->P_4096);
			bnEnd(&self->base.zrtp->P_4096_1);
			break;
		default:
			return zrtp_status_bad_param;
	}
										 ...
										 zrtp_dh_crypto_context_t *dh_cc)
{
	...
	ZRTP_LOG(1,(_ZTU_,"\tDH TEST: zrtp_dh_initialize() for %.4s was executed by %ums.\n", self->base.type, zrtp_time_now()-start_ts));
	return zrtp_status_ok;
}
									  ...
									  struct BigNum *pv)
{
	...
	ZRTP_LOG(1,(_ZTU_,"\tDH TEST: zrtp_dh_compute() for %.4s was executed by %ums.\n", self->base.type, zrtp_time_now()-start_ts));
    return zrtp_status_ok;
}
...
{
    zrtp_pk_scheme_t* dh4096 = zrtp_sys_alloc(sizeof(zrtp_pk_scheme_t));
	...
	if (!dh2048 || !dh3072 || !presh || !multi || !dh4096)	{
		if (dh4096) {
			zrtp_sys_free(dh4096);
		}
		if (dh2048) {
    ...
    zrtp_memset(dh4096, 0, sizeof(zrtp_pk_scheme_t));
    zrtp_memcpy(dh4096->base.type, ZRTP_DH4K, ZRTP_COMP_TYPE_SIZE);
	dh4096->base.id		= ZRTP_PKTYPE_DH4096;
    dh4096->base.zrtp	= zrtp;
    dh4096->sv_length	= 64;
    dh4096->pv_length	= 512;
    dh4096->base.init 	= zrtp_dh_init;
    dh4096->base.free	= zrtp_dh_free;
    dh4096->initialize	= zrtp_dh_initialize;
    dh4096->compute		= zrtp_dh_compute;
    dh4096->validate	= zrtp_dh_validate;
	dh4096->self_test	= zrtp_dh_self_test;
	zrtp_memcpy(zrtp->P_4096_data, P_4096_data, sizeof(P_4096_data));
    zrtp_comp_register(ZRTP_CC_PKT, dh4096, zrtp);
