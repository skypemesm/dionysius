...
{
	zrtp_hash_t *hash =  zrtp_comp_find(ZRTP_CC_HASH, ZRTP_HASH_SHA1, zrtp);
	if (NULL == hash) {
		ZRTP_LOG(1, (_ZTU_,"ERROR! can't find ZRTP_HASH_SHA1 component\n"));
	} else {
...
static zrtp_status_t dh_test(zrtp_global_t *zrtp)
{
	...
	pks = zrtp_comp_find(ZRTP_CC_PKT, ZRTP_PKTYPE_DH4096, zrtp);
	if (!pks) {
		ZRTP_LOG(1, (_ZTU_,"ERROR! can't find ZRTP_PKTYPE_DH4096 component\n"));
	} else {
		pks->self_test(pks);
	}
