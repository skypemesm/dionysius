...
typedef enum zrtp_crypto_comp_t
{
	...
	ZRTP_CC_ATL         = 5,
	// Digital Signature isn't available yet
	ZRTP_CC_SIG         = 6		/*!< digital signature scheme */
}zrtp_crypto_comp_t;
...
typedef enum zrtp_sigtype_id_t
{
	ZRTP_SIGTYPE_EC256P	= 1,
	ZRTP_SIGTYPE_EC384P	= 2,
	ZRTP_SIGTYPE_EC521P	= 3
} zrtp_sigtype_id_t; 
...
// Digital Signature isn't available yet
/*!
 * \brief Structure for describing the signature scheme
 * The ZRTP context field zrtp_stream#_sigscheme is initialized by the given type value
 * and used by libzrtp in public key exchange.
 */
struct zrtp_sig_scheme_t
{    
	zrtp_comp_t		base;

    /*! Generate long-term signature public key and secret key */
    zrtp_status_t	(*generate_key)( zrtp_sig_scheme_t *self,
								    zrtp_dsa_crypto_context_t *dsa_cc);
	
	/*! Sign a hash value */
    zrtp_status_t	(*sign)( zrtp_sig_scheme_t *self,
							 zrtp_dsa_crypto_context_t *dsa_cc,
							 uint8_t *hash,
							 uint32_t hash_len,
	...
	/*! Verify a signature */
   zrtp_status_t	(*verify)( zrtp_sig_scheme_t *self,
							   zrtp_dsa_crypto_context_t *dsa_cc,
							   uint8_t *hash,
							   uint32_t hash_len,
							   struct BigNum *dsasig);
	
    uint32_t		sv_length; /*!< Signature secret key value size in bytes */
    uint32_t		pv_length; /*!< Signature public key value size in bytes */
