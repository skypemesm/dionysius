...
static zrtp_status_t _create_sasrelay( zrtp_stream_t *stream,
									   ...
									   zrtp_string8_t* transf_sas_value,
									   ...
									   zrtp_packet_SASRelay_t* sasrelay )
{
	...
	/* (padding + sig_len + flags) + SAS scheme and value */
	const uint8_t encrypted_body_size = (2 + 1 + 1) + 4 + 4;

	zrtp_memset(sasrelay, 0, sizeof(zrtp_packet_SASRelay_t));

	/* generate a random initialization vector for CFB cipher  */
	zrtp_randstr(session->zrtp, sasrelay->iv, ZRTP_CFBIV_SIZE);
	...
	zrtp_memcpy( sasrelay->sas_scheme,
				 ...
				 ZRTP_COMP_TYPE_SIZE );
	if (transf_sas_value)
		zrtp_memcpy(sasrelay->mitm_sas, transf_sas_value->buffer, transf_sas_value->length);
	...
	{
		zrtp_string64_t hmac = ZSTR_INIT_EMPTY(hmac);
...
zrtp_status_t _zrtp_machine_process_sasrelay(zrtp_stream_t *stream, zrtp_rtp_info_t *packet)
{
	...
	if (session->secrets.matches & ZRTP_BIT_PBX) {
		if ( ( ((uint32_t) *sasrelay->sas_scheme) != (uint32_t)0x0L ) &&
			 ( ((uint32_t) *sasrelay->mitm_sas) != (uint32_t)0x0L ) )
		{
			char buff[256];
			session->sasbin.length = ZRTP_MITM_SAS_SIZE;
			zrtp_memcpy(session->sasbin.buffer, sasrelay->mitm_sas, session->sasbin.length);
	...
	} else if (0 != *(uint32_t*)sasrelay->mitm_sas) {
